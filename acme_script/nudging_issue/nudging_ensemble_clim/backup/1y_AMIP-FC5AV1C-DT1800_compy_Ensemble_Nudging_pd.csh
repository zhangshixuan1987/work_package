...
set irE = 5   # ensemble member: end   index
...
setenv INPUT_NUDGING /compyfs/zhan391/F20TRC5-CMIP6_NUDG_ISSUE/run

set casename = AMIP_CTRL_${COMPSET}_DT`printf "%04d" ${dtime}`

#-----------------
#setup run information
set dtime1      = 1800 
set run_refcase = AMIP_NDG_DATA_F20TRC5-CMIP6_DT`printf "%04d" ${dtime1}`
set run_refdate = "2010-01-01"
...
if ($run_model > 0) then
 ...
 while ( $ir <= $irE )
   ...
   set runcase   = ${casename}_en${irstring}
...
###copy the required restart files to the directory for branch run#####
  ...
  #ln -sf ${initDir}${lndrhin}   $RUNDIR/${lndrhin}
   ln -sf ${initDir}${cicerin}  $RUNDIR/${cicerin}
   ln -sf ${initDir}${cplrin}   $RUNDIR/${cplrin}
  #ln -sf ${initDir}${docnrin}  $RUNDIR/${docnrin}
  ...
  #
   ...
   ./xmlchange  -file env_batch.xml -id JOB_WALLCLOCK_TIME -val "16:00:00"
...
!history_microphysics = .true.
 ...
 fincl1               = 'OMEGA500','RH700','TH7001000','THE7001000',
                        'DPDLFLIQ','DPDLFICE','DPDLFT','DPDLFNUMLIQ','DPDLFNUMICE',
!                       'Nudge_U', 'Nudge_V','Nudge_T',
 fincl2               = 'FLNT','FSNT','FLNTC','FSNTC',
                        ...
                        'DPDLFLIQ','DPDLFICE','DPDLFT','DPDLFNUMLIQ','DPDLFNUMICE',
                        'TH7001000','THE7001000',
!                       'Nudge_U', 'Nudge_V','Nudge_T',
 fincl3               = 'PS','U','V','T','Q','Z3','OMEGA','TH7001000','THE7001000',
                        'DPDLFLIQ','DPDLFICE','DPDLFT','DPDLFNUMLIQ','DPDLFNUMICE',
!                       'Nudge_U', 'Nudge_V','Nudge_T',
...
cat <<EOF >> user_nl_cam
 ...
 !.......................................................
  ...
  Nudge_Curr   = .True.
  Num_Slice    = 1
  Nudge_Path   = '${INPUT_NUDGING}/${run_refcase}/'
