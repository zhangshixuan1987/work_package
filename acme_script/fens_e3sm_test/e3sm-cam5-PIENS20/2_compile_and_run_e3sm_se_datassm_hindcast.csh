...
set irE = 33    # ensemble member: end   index
####################################################################
# Fetch code
####################################################################
...
set case_setup_script = "create_and_setup_bundled_branch.csh"
...
if ($run_model > 0) then
  ...
  if ( $ir == 1 )then
    set run_refdate = "2009-01-27"
    set run_reftod  = "00000"
  endif
  if ( $ir == 2 )then
    set run_refdate = "2009-01-28"
    set run_reftod  = "00000"
  endif
  if ( $ir == 3 )then
    set run_refdate = "2009-01-29"
    set run_reftod  = "00000"
  endif
  if ( $ir == 4 )then
  ...
  if ( $ir >= 6 )then
    @ irr = $ir - 5
    set dystr = `printf "%02d" ${irr}`
    set run_refdate = "2009-02-${dystr}"
    set run_reftod  = "00000"
  endif
  ...
  set dtime1      = 1800
  set run_refcase = PIDACAM5_ENS20_F20TRC5-AMIP_f19_g16_DT1800  
  set initDir     = ${INPUT_SPIN}/${run_refcase}/rest/$run_refdate-$run_reftod/
   ...
   set casename = PIHIND_5d_ENS20_F20TRC5-AMIP_f19_g16_DT`printf "%04d" ${dtime}`"_"${irstring}
...
while ( $ininst <= $NINST )
 ...
 if ( $NINST <= 1 ) then  #(we have single-instances)
  ...
  #set lndrin   = "$run_refcase.clm2.rh0.$run_refdate-$run_reftod.nc"
   ...
   cp -rp ${initDir}${camiin}   $RUNDIR/${camiin}
   cp -rp ${initDir}${camrin}   $RUNDIR/${camrin}
   cp -rp ${initDir}${camrsin}  $RUNDIR/${camrsin}
  #cp -rp ${initDir}${camrhin}  $RUNDIR/${camrhin}
   cp -rp ${initDir}${lndiin}   $RUNDIR/${lndiin}
  #cp -rp ${initDir}${lndrin}   $RUNDIR/${lndrin}
   cp -rp ${initDir}${cicerin}  $RUNDIR/${cicerin}
   cp -rp ${initDir}${cplrin}   $RUNDIR/${cplrin}
   cp -rp ${initDir}${docnrsin} $RUNDIR/${docnrsin}
  ...
  ####change the start date to let the model output correctly. 
   ncap2 -O -v -s 'time=0.0;rst_start_ymd=rst_curr_ymd;rst_start_tod=rst_curr_tod;rst_ref_ymd=rst_curr_ymd;rst_ref_tod=rst_curr_tod' $RUNDIR/${camrin} $RUNDIR/${camrin}_tmp
   ncks -A -C -v time,rst_start_ymd,rst_start_tod,rst_ref_ymd,rst_ref_tod $RUNDIR/${camrin}_tmp $RUNDIR/${camrin}
   rm -rvf $RUNDIR/${camrin}_tmp

   ncap2 -O -v -s 'timemgr_rst_nstep_rad_prev=0;timemgr_rst_start_ymd=timemgr_rst_curr_ymd;timemgr_rst_start_tod=timemgr_rst_curr_tod;timemgr_rst_ref_ymd=timemgr_rst_curr_ymd;timemgr_rst_ref_tod=timemgr_rst_curr_tod' $RUNDIR/${lndiin} $RUNDIR/${lndiin}_tmp
   ncks -A -C -v timemgr_rst_nstep_rad_prev,timemgr_rst_start_ymd,timemgr_rst_start_tod,timemgr_rst_ref_ymd,timemgr_rst_ref_tod $RUNDIR/${lndiin}_tmp $RUNDIR/${lndiin}
   rm -rvf $RUNDIR/${lndiin}_tmp
  ...
  if ( $ininst == 1 ) then
   ...
   echo "${cplrin}"     >! $RUNDIR/rpointer.drv
   ncap2 -O -v -s 'seq_timemgr_start_ymd=seq_timemgr_curr_ymd;seq_timemgr_start_tod=seq_timemgr_curr_tod;seq_timemgr_ref_ymd=seq_timemgr_curr_ymd;seq_timemgr_ref_tod=seq_timemgr_curr_tod' $RUNDIR/${cplrin} $RUNDIR/${cplrin}_tmp
   ncks -A -C -v  seq_timemgr_start_ymd,seq_timemgr_start_tod,seq_timemgr_ref_ymd,seq_timemgr_ref_tod $RUNDIR/${cplrin}_tmp $RUNDIR/${cplrin}
   rm -rvf $RUNDIR/${cplrin}_tmp
  endif
...
!history_microphysics = .true.
 ...
 fincl2               = 'AODVIS','angstrm','cod','cdr','cdnc',
  ...
  srf_emis_specifier             = 'DMS       -> /qfs/people/zhan524/share/for_shixuan/aerocom_mam3_dms_surf_1849-2010_c20100902.nc',
