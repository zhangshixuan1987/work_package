...
set irE = 4    # ensemble member: end   index
####################################################################
# Fetch code
####################################################################
...
set casename = TESTHIND_5d_ENS20_${COMPSET}_${RESOLUTION}_DT`printf "%04d" ${dtime}`
set MULTI_DRIVER = ' '
...
if ($run_model > 0) then
  ...
  set initDir     = ${INPUT_SPIN}/${run_refcase}/rest/$run_refdate-$run_reftod/
   ...
   set casename = TESTHIND_5d_ENS20_F20TRC5-AMIP_f19_g16_DT`printf "%04d" ${dtime}`"_"${irstring}
