...
user_eam_nl() {
  ex user_nl_eam <<ex_end
  g;ncdata ;s;=.*;= \"${1}\";
  g;inithist ;s;=.*;= \'${2}-HOURLY\';
  g;inithist_all ;s;=.*;= .true.;
  wq
ex_end
}
user_elm_nl() {
  ex user_nl_elm <<ex_end
  g;finidat ;s;=.*;= \"${1}\";
  g;check_finidat_year_consistency ;s;=.*;= ${2};
  g;check_dynpft_consistency ;s;=.*;= ${3};
  g;check_finidat_fsurdat_consistency ;s;=.*;= ${4};
  g;check_finidat_pct_consistency ;s;=.*;= ${5};
  wq
ex_end
}
user_mosart_nl() {
  ex user_nl_mosart <<ex_end
  g;finidat_rtm ;s;=.*;= \"${1}\";
  wq
ex_end
}
user_mpassi_nl() {
  ex user_nl_mpassi <<ex_end
  g;config_calendar_type ;s;=.*;= 'gregorian';
  g;config_start_time ;s;=.*;= '${1}_`printf "%02d" ${2}`';
...
user_mpaso_nl() {
  #add namelist for mpaso
  ex user_nl_mpaso <<ex_end
  g;config_calendar_type ;s;=.*;= 'gregorian';
  g;config_do_restart ;s;=.*;= .true.;
  g;config_start_time ;s;=.*;= '${1}_`printf "%02d" ${2}`';
  g;config_output_reference_time ;s;=.*;= '${3}';
