...
if [ -e "${DART_WORKDIR}/eam_dart_noda.nml" ]; then
  ${COPY} ${DART_WORKDIR}/eam_dart_noda.nml input.nml  || exit 04
  sed -i "/#/d;/^\!/d;/^[ ]*\!/d" input.nml
  sed -i '1,1i\WARNING: Changes to this file will be ignored. \n Edit \$DART_WORKDIR/eam_dart_noda.nml instead.\n\n\n'  input.nml
else
  echo "ERROR ... DART required file ${DART_WORKDIR}/eam_dart_noda.nml not found ... ERROR"
  echo "ERROR ... DART required file ${DART_WORKDIR}/eam_dart_noda.nml not found ... ERROR"
  exit 05
fi
