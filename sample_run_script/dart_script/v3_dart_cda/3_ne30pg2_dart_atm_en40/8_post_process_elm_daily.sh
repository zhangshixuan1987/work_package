...
# Variables
var1_list=(H2OSNO QRUNOFF SNOW FSNO FSR FLDS FSDS)
  ...
  for year in $(seq ${sy} ${ey}); do
    for month in $(seq 1 12); do
      ...
      for dd in `seq 1 ${nday}`;do 
         xfiles=${CASE_NAME}.${hist1}.`printf "%04d" ${year}`-`printf "%02d" ${month}`-`printf "%02d" ${dd}`*.nc
         echo ${xfiles[@]}
         ncra ${xfiles[@]} ftmp_`printf "%02d" ${dd}`.nc
      done 
      ls ftmp_*.nc > input1.txt
      ...
      for year in $(seq ${sy} ${ey}); do
        ...
        for var in "${var1_list[@]}"; do
          ...
          if [ "${#ffiles1[@]}" -gt 0 ]; then
            ...
            ncremap -P elm -m "${MAP_FILE}" "${outfile}" "${ts_dest1}/${outfile}"
          else
