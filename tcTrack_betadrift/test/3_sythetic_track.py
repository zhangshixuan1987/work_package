...
for i in np.arange(nexp): 
    ...
    for j in np.arange(nyear): 
        ...
        sythetic_track = np.empty((nvars,nrdtrk,max_steps),dtype=float)
        sythetic_track[:,:,:] = np.nan
        #print(sythetic_track)

        storm_time  = np.empty((nrdtrk,max_steps),dtype='U20')
        storm_time[:,:] = yrdoy2date(1,1)
        #print(stime)

        #do track forecast 
        for i in np.arange(3): #nrdtrk):
            fsamp = sample[i,:]
            fstrks,fstimes = \
                    sythetic_track_forecst(nvars,max_steps,time_step,fsamp,land_mask,\
                                           rglat,rglon,year,fu200,fv200,fu850,fv850)

            sythetic_track[:,i,:] = fstrks
            storm_time[i,:]       = fstimes 
        ...
        ds = xr.Dataset({"strks": (("nvars","nrdtrk","max_steps"), sythetic_track),
                         "stime": (("nrdtrk","max_steps"),storm_time),
                         "sname": (("nvars"),varlist)},
                        coords={
                            "nvars": np.arange(nx),
                            "nrdtrk": np.arange(ny),
        ...
        file_out = 'sythetic_track_forecast_'+str(year)+'.nc'
