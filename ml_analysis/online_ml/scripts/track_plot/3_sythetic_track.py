...
def sythetic_track_forecst(nrdtrk,max_steps,sample,land_mask,rglat,rglon,year,u200,v200,u850,v850):
    ...
    for i in np.arange(nrdtrk):
        ...
        #initial condition
        step         = 0
        storm_lat    = sample[i,0].data
        storm_lon    = sample[i,1].data
        storm_time   = sample[i,2].data
        time_on_land = 0.0
        ...
        while ((storm_lat > (rglat[0]+4)) and (storm_lat < rglat[1]) and \
               (storm_lon > rglon[0]) and (storm_lon < (rglon[1]-5)) and \
            ...
            strks[:,i,step] = [storm_time, storm_lat, storm_lon, u.data, v.data, xu200.data, xv200.data, xu850.data, xv850.data]
            stime[i,step]   = yrdoy2date(year,storm_time)
            #print(strks[:,i,step])
...
for i in np.arange(nexp): 
    ...
    for j in np.arange(nyear): 
        ...
        #strks,stime,sname
        nx,ny,nz = strks.shape
        ds = xr.Dataset({"strks": (("nvars","nrdtrk","max_steps"), strks),
                         "stime": (("nrdtrk","max_steps"),stime),
                         "sname": (("nvars"),sname)},
                        coords={
                            "nvars": np.arange(nx),
                            "nrdtrk": np.arange(ny),
                            "max_steps": np.arange(nz),
                            },
                        )

        file_out = 'sythetic_track_forecast'+str(year)+'.nc'
